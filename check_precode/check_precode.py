...
for index, row in df.iterrows():
    if row['*适用项目'] == 'X-Public':
        # 判断另外指定行是否包含某个字段
        if '语音Xpublic' in str( df.loc[index, 'precode'] ):
            # 执行相应的操作
            print( f"row {index}" + ' ' + row['precode'] + " contains " + '语音Xpublic' )
        else:
            print( f"row {index}" + ' ' + row['precode'] + ' doesnot contains ' '语音Xpublic' )
    if row['*适用项目'] == 'X-Public-Max':
        # 判断另外指定行是否包含某个字段
        if '语音Xmax' in str( df.loc[index, 'precode'] ):
            # 执行相应的操作
            print( f"row {index}" + ' ' + row['precode'] + ' contains ' '语音Xmax' )
        else:
            print(f"row {index}"  + ' ' + row['precode'] + ' doesnot contains ' '语音Xmax' )
    if row['*适用项目'] == 'X-Public-Pro':
        # 判断另外指定行是否包含某个字段
        if '语音Xpro' in str( df.loc[index, 'precode'] ):
            # 执行相应的操作
            print(f"row {index}" + ' ' + row['precode'] + ' contains ' '语音Xpro' )
        else:
            print(f"row {index}" + ' ' + row['precode'] + ' doesnot contains ' '语音Xpro' )
